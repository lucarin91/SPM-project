...
def plot(seq, data,
         leg_name=['0 sin', '1000 sin', '4500 sin', '10000 sin', '45000 sin']):
    ...
    for val in data:
        ...
        x = [e[0] for e in val]
        y = [e[1] for e in val]
        ax1.plot(x, y, label=str(leg_name[i]), linewidth=0.85,
    ...
    for val in data:
        ...
        ax2.plot(x, y, label=str(leg_name[i]), linewidth=0.85,
                 alpha=0.8, marker=marker[i], markersize=5)
        ax2.plot(x_ideal, x_ideal, label='Opt', linewidth=1.95,
                 alpha=0.2, color='gray', linestyle='dashed')

        inset2.plot(x, y, marker=marker[i],
                    linewidth=0.85, alpha=0.8, markersize=3)
        inset2.plot(x_ideal, x_ideal, linewidth=1.95, alpha=0.4,
    ...
    for val in data:
        ...
        ax3.plot(x, y, label=str(leg_name[i]), linewidth=0.85,
                 alpha=0.8, marker=marker[i], markersize=5)

        ax3.plot(x_ideal, x_ideal, linewidth=1.95, alpha=0.2,
                 color='gray', linestyle='dashed')

        inset3.plot(x, y, marker=marker[i],
                    linewidth=0.85, alpha=0.8, markersize=3)
        inset3.plot(x_ideal, x_ideal, linewidth=1.95, alpha=0.4,
                    color='gray', linestyle='dashed')
    ...
    for val in data:
        ...
        for z in range(0, len(y)):
            if y[z] > 1:
                y[z] = 1
        ax4.plot(x, y, label=str(leg_name[i]), linewidth=0.85,
                 alpha=0.8, marker=marker[i], markersize=5)
        x_new = np.linspace(1, 70, 100)
        y_new = [1 for val in x_new]

        plt.xticks(np.arange(min(x) - 1, max(x) + 1, 5.0))
        plt.xlabel(r'$\textit{Threads}$')
        plt.ylabel(r'$\textit{Efficiency}$')
        # ax4.xaxis.grid(True)
        ax4.yaxis.grid(True)
        ax4.set_ylim([0, 1.1])
        i += 1
