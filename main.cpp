...
int main() {
    ...
    gr->add("test", {
            Statement([](t_in in_ptr) -> t_out {
                ...
                return Token_value<int>(4, in.value * 2);
            }, {3}, 4),

            Statement([](t_in in_ptr) -> t_out {
                auto &in = static_cast<Token_value<int> &> (in_ptr[0]);
                return Token_value<int>(5, in.value * 3);
            }, {3}, 5),

            Statement([](t_in in_ptr) -> t_out {
                auto &in1 = static_cast<Token_value<int> &> (in_ptr[0]);
                auto &in2 = static_cast<Token_value<int> &> (in_ptr[1]);
                return Token_value<int>(3, in1.value + in2.value);
            }, {1, 2}, 3)
    });
    ...
    for (int i = 0; i < 30; i++) {
        inFactory.start("test", {Token_value<int>(1, 1),
                                 Token_value<int>(2, 1)}, drain);
    }
    return 0;
}