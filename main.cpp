...
int main(int argc, char* argv[]) {
    ...
    gr->add("test", {
            Statement([](t_in in_ptr) -> t_out {
                ...
                return Token_value<int>(in.value * 2);
            }, {3}, 4),

            Statement([](t_in in_ptr) -> t_out {
                auto &in = static_cast<Token_value<int> &> (in_ptr[0]);
                return Token_value<int>(in.value * 3);
            }, {3}, 5),

            Statement([](t_in in_ptr) -> t_out {
                auto &in1 = static_cast<Token_value<int> &> (in_ptr[0]);
                auto &in2 = static_cast<Token_value<int> &> (in_ptr[1]);
                return Token_value<int>(in1.value + in2.value);
            }, {1, 2}, 3)
    });
    ...
    Drainer drain = [&prev,&sumTS](shared_ptr<Token> t) {
        auto now = Helper::gettime();
        sumTS += now - prev;
        prev = now;
        cout <<"drainer "<< t->id << endl;
